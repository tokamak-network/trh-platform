...
- RegisterCandidate 기능 — 아직 개발 중이므로 테스트 범위에서 제외
- Helm/Kubernetes 배포 — 클라우드 인프라 테스트는 범위 외
- AWS 실제 인증/API 호출 — Mock으로 대체, 실제 AWS 연동 불필요
...
- 배포 대상 인프라: Local Docker (로컬 개발) 및 AWS EC2 (Terraform 기반 클라우드) 두 경로 모두 테스트 대상
