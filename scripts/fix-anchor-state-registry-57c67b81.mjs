/**
 * Fix AnchorStateRegistry for deployment 57c67b81
 *
 * Root cause: tokamak-deployer embeds pre-compiled contract bytecode; patching
 * AnchorStateRegistry.sol in the cloned source has no effect on what gets deployed.
 * The deployed impl (0x837D5572) lacks setInitialAnchorState, so initGenesisAnchorState
 * pre-flight simulation reverts → deploy-local-infra fails before orchestrateDRBOperators.
 *
 * Fix: StorageSetter pattern — write genesis anchor root directly to proxy storage,
 * bypassing the missing setInitialAnchorState. Then restore original impl.
 * On retry, initGenesisAnchorState idempotency check finds non-zero root and skips.
 *
 * Usage:
 *   node scripts/fix-anchor-state-registry-57c67b81.mjs
 */
...
// 57c67b81 deployment addresses (from deploy-output.json / forge logs)
const PROXY_ADMIN                = '0x1447CbC4be8a1e6729688A75CA0C113055CaF359';
const ANCHOR_STATE_REGISTRY      = '0x0Ed6ff9d6de2833858EF8b00dFDe34c73c5727f9';
const ANCHOR_STATE_REGISTRY_IMPL = '0x837D5572cDdF221956Cc52fDC59043F54d7EB54e';
...
async function main() {
  ...
  if (finalRoot.toLowerCase() === genesisOutputRoot.toLowerCase()) {
    ...
    console.log('rollup.json genesis.l2.hash was also patched (RC1 fix).');
